...
export function StaffChatView({ state = "default" }: { state?: UiState }) {
  ...
  if (state === "loading")
    return <LoadingState title="Carregando chat da equipe" />;
  if (state === "empty")
    return <EmptyState title="Sem conversas em andamento" />;
  if (state === "error") return <ErrorState title="Erro ao carregar chat" />;
  if (state === "forbidden")
    return <ForbiddenState title="Acesso negado ao chat da equipe" />;

  const rooms = chatMock.rooms.filter((room) => {
    if (filter === "nao-lidas") return room.unreadCount > 0;
    if (filter === "pacientes")
      return room.kind === "patient" && !room.archived;
    if (filter === "internas") return room.kind === "internal";
    if (filter === "arquivadas") return room.archived;
    return true;
  });
  const selectedRoomId = rooms.some((room) => room.id === roomId)
    ? roomId
    : (rooms[0]?.id ?? "");
  const messages = chatMock.messages.filter((m) => m.roomId === selectedRoomId);
  const context = chatMock.contexts.find(
    (ctx) => ctx.roomId === selectedRoomId,
  );
  const selectedRoom = chatMock.rooms.find(
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[320px_1fr_320px]">
      ...
      <ChatPatientContextPanel context={context} room={selectedRoom} />
    </div>
  );
}
