...
export default function FinancePage() {
  ...
  if (state === "empty")
    return (
      <DashboardShell active="Financeiro">
        <EmptyState title="Sem dados financeiros" />
      </DashboardShell>
    );
  if (state === "error")
    return (
      <DashboardShell active="Financeiro">
        <ErrorState title="Falha ao carregar financeiro" />
      </DashboardShell>
    );
  if (state === "forbidden")
    return (
      <DashboardShell active="Financeiro">
        <ForbiddenState title="Acesso negado ao financeiro" />
      </DashboardShell>
    );
  ...
  return (
    <DashboardShell active="Financeiro">
      <PageHeader
        ...
        description="Visão consolidada por unidade com fluxos financeiros simulados."
      />
      <div className="ep-kpi-grid">
        {financeKpis.map((kpi) => (
          <StatCard key={kpi.label} title={kpi.label} value={kpi.value} />
        ))}
      </div>
      <Card className="mt-4">
        <CardHeader>
          <CardTitle>Conector Asaas (planejado)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-sm text-slate-600">
          <p>
            Status: <strong>{asaasIntegrationPlan.status}</strong>
          </p>
          <p>Execução: server-side only</p>
          <p>Chave API no browser: bloqueada</p>
          <p>RLS: {asaasIntegrationPlan.rls}</p>
          <Button className="mt-2" onClick={() => setOpenPaymentModal(true)}>
            Registrar pagamento
          </Button>
        </CardContent>
      </Card>
