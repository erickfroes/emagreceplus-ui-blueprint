...
export default function ReceivablesPage() {
  ...
  const selectedReceivable =
    receivables.find((item) => item.id === selectedReceivableId) ??
    receivables[0];
  ...
  return (
    <DashboardShell active="Financeiro">
      <PageHeader
        ...
        description="Gestão visual de recebimentos com integração Asaas simulada e server-side planejada."
      />
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {receivables.map((item) => (
              <div key={item.id} className="space-y-2 rounded-2xl border p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold">{item.patient}</p>
                    <p className="text-sm text-slate-500">
                      {item.service} • vence em {item.dueDate}
                    </p>
                  </div>
                ...
                <p className="text-xs text-slate-500">
                  Cobrança:{" "}
                  {item.allowedBillingTypes
                    .map((type) => billingLabels[type])
                    .join(" / ")}{" "}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    Registrar pagamento desta cobrança
                  </Button>
                  <Button
                    variant="ghost"
                    onClick={() => {
                  ...
                  >
                    Ações da linha: registrar pagamento
                  </Button>
                </div>
              </div>
            ))}
            ...
            <p className="text-xs text-slate-500">
              Fluxo Asaas em modo <strong>{asaasIntegrationPlan.status}</strong>
              . Sem chave no browser, webhook idempotente e sem baixa dupla.
            </p>
          </CardContent>
        </Card>
      </UiStateView>
      ...
      <ConfirmDialog
        ...
        description="Deseja marcar os itens selecionados como pagos?"
